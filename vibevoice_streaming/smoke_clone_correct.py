"""Re-test cloning with USER'S WINNING SETTINGS:
- model: VibeVoice-1.5B
- attn_implementation: flash_attention_2 (CRITICAL — sdpa gives gibberish on Arabic)
- diffusion_steps: 40
- cfg_scale: 1.5
- seed: 42
- use_sampling: False
...
OUT_DIR = "/workspace/refs/out_correct"
...
model.eval()
model.set_ddpm_inference_steps(num_steps=40)  # <-- USER'S WINNING SETTING
...
def gen_segment(text, idx, voice_samples):
    """Generate one segment using cached voice with USER'S WINNING SETTINGS."""
    ...
    with torch.no_grad():
        out = model.generate(
            **inputs,
            max_new_tokens=None,
            cfg_scale=1.5,  # <-- USER'S WINNING
            tokenizer=processor.tokenizer,
            generation_config={'do_sample': False, 'num_beams': 1, 'temperature': 0.95, 'top_p': 0.95},
            verbose=False,
        )
...
print(f"\n[3/4] === one-shot baseline (premium quality) ===")
t0 = time.time()
